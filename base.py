...
"""Line 129 147 Reg; 69 kwargs init; """
...
class ABSBaseLayer(tf.keras.layers.Layer):
    ...
    @property
    def last_layer(self):
        '''The layer must have exactly one inbound connection:
        It is used only once in the model.'''
...
class HLayerBase(ABSBaseLayer):
    ...
    def __init__(self, kq_conf=None, paq_conf=None, beta=0., **kwargs):
        ...
        super().__init__(**kwargs) # Keras Layer parent class automatically sets self.activation

    def build(self, input_shape):
        super().build(input_shape)
        """ ^ just runs tf.keras.layers.Layer.build(input_shape),
        which only marks the layer as "built",
        allowing it to participate in the model."""
        self.post_build(input_shape)
        """post_build() is where the layer is actually initializing weights
    ...
    def init_quantizers(self, input_shape):
        ...
        if self._has_kernel:
            ...
            # kq.init_bw is the initial value to fill the weight with
            kq.build(fbw)
